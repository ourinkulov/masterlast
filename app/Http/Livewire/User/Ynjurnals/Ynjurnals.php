...
namespace App\Http\Livewire\User\Ynjurnals;


use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\DB;
use Livewire\Component;
use App\Models\Ynjurnal;

class Ynjurnals extends Component
{
    public $complesid, $userID, $bosqichid, $ynid, $baxo,$gid;
    public $jurnalid;
    public $sid,$baho;
    public function render()
    {
        ...
        $joriy = DB::table('v_yakuniynazorat')
            ->select('id','yakuniyname')
...
//                   ->get();
        ...
        if ($this->ynid != '')
        {
            $students2 = DB::table('v_ynjurnals')
                ->select('id','name','baho','jnName','jnjurnalid')
                ->where('gid',$this->gid)
                ->where('jnid',$this->ynid)
                ->orderBy('id','DESC')
                ->get();
            //dd($students2);
        }
        return view('livewire.user.ynjurnals.ynjurnals',compact(['comples','bosqich','joriy','guruh','students','students2']))->extends('layouts.userApp2');
    ...
    public function store()
    {
        ...
        if ($valid)
        {
            ...
            $userCheck=DB::table('v_ynjurnals')
                ->where('jnid',$this->ynid)
                ->where('comples_id',$this->complesid)
                ->where('id',$this->sid)
                ->get();
    ...
    public function renew()
    {
        ...
        if ($valid)
        {  //dd($this->jurnalid);
            $joriyjurnal = Ynjurnal::find($this->jurnalid);
